...
public:
	...
	void Initialize(const FVector& InExtent, const float InConeAngle, const float InInnerRingPercent, const EFaction InHostility, const float InDetonationTime, 
		const FLinearColor& InIndicatorColor, UTexture2D* InIndicatorTexture, const float InIntensity);
	virtual void Tick(float DeltaSeconds) override;

	UFUNCTION(BlueprintAuthorityOnly, Category = "Decal")
	...
	UFUNCTION(BlueprintAuthorityOnly, Category = "Decal")
	void SetDetonationTime(const float NewDetonationTime);
	...
	FVector GetBoxInfo(FVector& OutExtent, FRotator& OutRotation) const;
	UFUNCTION(BlueprintPure, Category = "Test")
	float GetDistanceAtAngle(const float Angle) const { return GetRadiusAtAngle(Angle); }
