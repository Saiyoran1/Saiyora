...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Abilities")
	virtual FAbilityEvent UseAbility(TSubclassOf<UCombatAbility> const AbilityClass);
	UFUNCTION(BlueprintCallable, Category = "Abilities")
...
protected:
	...
	void MulticastAbilityTick(FAbilityEvent const& CastEvent, FCombatParameters const& BroadcastParams);
	UFUNCTION()
	virtual void CompleteCast();
	UFUNCTION(NetMulticast, Unreliable)
	void MulticastAbilityComplete(FAbilityEvent const& CastEvent);
	void EndCast();
