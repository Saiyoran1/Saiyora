...
void AGroundAttack::Initialize(const FVector& InExtent, const float InConeAngle, const float InInnerRingPercent,
	const EFaction InHostility, const float InDetonationTime, const FLinearColor& InIndicatorColor,
	UTexture2D* InIndicatorTexture, const float InIntensity)
{
	...
	SetDetonationTime(InDetonationTime);
...
void AGroundAttack::SetDecalExtent(const FVector& NewExtent)
{
	...
	DecalExtent = FVector(FMath::Max(1.0f, NewExtent.Z), FMath::Max(1.0f, NewExtent.Y), FMath::Max(1.0f, NewExtent.X));
	OnRep_DecalExtent();
}

void AGroundAttack::OnRep_DecalExtent()
{
	SetActorScale3D(DecalExtent / 100.0f);
}
...
void AGroundAttack::SetDetonationTime(const float NewDetonationTime)
{
	if (GetLocalRole() != ROLE_Authority)
	{
		return;
...
FVector AGroundAttack::GetBoxInfo(FVector& OutExtent, FRotator& OutRotation) const
{
	OutExtent = OuterBox->GetScaledBoxExtent();
	OutRotation = OuterBox->GetComponentRotation();
	return OuterBox->GetComponentLocation();
}
